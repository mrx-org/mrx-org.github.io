...
The [ToolAPI](../index.md) exchanges data between clients and tools using a canonical set of typed values. Upon calling a tool, the client sends a `Value` as input; when finished, the tool returns a `Value` as output.
...
See also: [Implementations](../implementations.md) for language-specific value handling.
